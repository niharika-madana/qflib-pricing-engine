...
/** Price of quanto European option in the Black-Scholes model */
                              ...
                              double assetVol, double fxVol, double correl)
{
  ...
  double phi = payoffType;
  // quanto forward: S * exp((rf - q + rho*sigS*sigQ) * T)
  double fwd = spot * exp((growthRate - divYield + correl * assetVol * fxVol) * timeToExp);
  double sigT = assetVol * sqrt(timeToExp);
  double epsilon = 1.0e-012;
  double d1 = log(fwd / strike) / (sigT + epsilon) + 0.5 * sigT;
  double d2 = d1 - sigT;
  NormalDistribution normal;
  ...
  return phi * df * (fwd * nd1 - strike * nd2);
}
