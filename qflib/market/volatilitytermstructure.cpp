...
{
  QF_ASSERT(tMat >= 0.0, "spot volatilities for negative times not allowed");
  if (tMat == 0.0)
...
{
  QF_ASSERT(tMat1 >= 0.0, "forward volatilities for negative times not allowed");
  QF_ASSERT(tMat1 <= tMat2, "maturities are out of order");
  if (tMat1 == tMat2)
