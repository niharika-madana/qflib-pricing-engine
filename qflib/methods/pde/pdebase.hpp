...
#include <qflib/methods/pde/pdegrid.hpp>
...
public:
    ...
    spaccrycs_(accrualYieldCurves), divyields_(divyields), vols_(vols)
  {
    ...
    QF_ASSERT(spots_.size() == nAssets_, "missing or redundant spots!");
    QF_ASSERT(spaccrycs_.size() == nAssets_, "missing or redundant accrual yield curves!");
    QF_ASSERT(divyields_.size() == nAssets_, "missing or redundant dividend yields!");
    QF_ASSERT(vols_.size() == nAssets_, "missing or redundant volatilities!");
  }
...
protected:
  ...
  // quanto adjustment parameters; used only when isQuanto_ is true
  bool isQuanto_ = false;     // if true, apply quanto drift adjustment
  double fxVol_ = 0.0;        // FX (payoff/local) volatility
  double correl_ = 0.0;       // asset-FX correlation
