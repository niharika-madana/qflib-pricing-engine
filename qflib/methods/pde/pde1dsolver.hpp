...
public:
  ...
  /** Quanto ctor.
      The asset price path is simulated in the payoff currency measure with drift
      (rf - q + rho*sigmaS*sigmaQ), where rf is the growth rate read from
      growthYieldCurve. Cash flows are discounted with discountYieldCurve (rd).
  */
  Pde1DSolver(SPtrProduct product,
  ...
  : PdeBase(product), results_(results), storeAllResults_(storeAllResults)
  {
    ...
    divyields_.push_back(divyield);
    vols_.push_back(vol);
    // enable the quanto drift adjustment in PdeBase::solve
    isQuanto_ = true;
    fxVol_ = fxVol;
    correl_ = correl;
  }
