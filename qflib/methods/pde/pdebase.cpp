...
{
  QF_ASSERT(nAssets_ == params.nSpotNodes.size(),
...
void PdeBase::solve(PdeParams const& params)
{
  ...
  // compute the conditional forward factors from step to step
  // the row index is the time, the column index is the asset
  // when pricing a quanto option, the effective drift of the asset in the payoff
  // currency measure is (rf - q + rho*sigmaS*sigmaQ) => add the convexity term
  // rho*sigmaS*sigmaQ to the standard (rf - q) drift
  Matrix fwdFactors(nSteps_, nAssets_);
  for (size_t j = 0; j < nAssets_; ++j) {
    SPtrYieldCurve spyc = spaccrycs_[j];
    double divyld = divyields_[j];
    for (size_t i = 0; i < nSteps_ - 1; ++i) {
      ...
      double T2 = timesteps_[i + 1];
      double fwdRate = spyc->fwdRate(T1, T2);
      double quantoAdj = 0.0;
      if (isQuanto_) {
        // only supported for single-asset 1D PDE (where j == 0)
        quantoAdj = correl_ * fxVol_ * fwdVols(i, j);
      }
      fwdFactors(i, j) = exp((fwdRate - divyld + quantoAdj) * (T2 - T1));
    }
  }
