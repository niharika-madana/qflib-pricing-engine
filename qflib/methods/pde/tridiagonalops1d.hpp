...
/** Base class representing a tridiagonal operator arising in discretization of
    1-dimensional PDEs.
*/
template <class ARRAY = qf::Vector>
...
public:
  ...
  {
    QF_ASSERT(a.N_ == b.N_, "TridiagonalOperator1D: cannot add two operators of different sizes");
  ...
  friend TridiagonalOp1D operator-(TridiagonalOp1D const& a,
                                   TridiagonalOp1D const& b)
  {
    QF_ASSERT(a.N_ == b.N_, "TridiagonalOperator1D: cannot subtract two operators of different sizes");
    TridiagonalOp1D<ARRAY> res;
...
template <typename ARRAY = qf::Vector >
...
double TridiagonalOp1D<ARRAY>::adjustForLowerBoundaryCondition(
                                          ...
                                          double upAdjust)
{
  QF_ASSERT(diag_.size() >= 4, "TridiagonalOperator1D: grid is too small!");
  switch (degree) {
  case 0:
  ...
  case 4:
    ...
    return 0.0;
  default:
    QF_ASSERT(0, "TridiagonalOperator1D: invalid degree for boundary condition");
  }
}
...
{
  QF_ASSERT(N_ == rhs.N_, "TridiagonalOperator1D: cannot add two operators of different sizes");
...
TridiagonalOp1D<ARRAY>::operator-=(TridiagonalOp1D<ARRAY1> const& rhs)
{
  QF_ASSERT(N_ == rhs.N_, "Cannot subtract two operators of different sizes");
  for (size_t i = 0; i < lower_.size(); ++i) {
    lower_[i] -= rhs.lower_[i];
    diag_[i] -= rhs.diag_[i];
    upper_[i] -= rhs.upper_[i];
