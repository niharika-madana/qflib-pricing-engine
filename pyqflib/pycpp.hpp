...
/**
@file  pycpp.hpp
@brief Utility functions for Python to C++ bidirectional type conversion, including numpy types.
...
if (!(condition)) { \
    ...
    throw std::runtime_error("pyqflib error: " #condition); \
    else \
...
{
    PyObject* arr = PyArray_FROM_OTF(pobj, NPY_LONG, NPY_ARRAY_IN_ARRAY);
    if (arr == nullptr)
...
static PyObject* asPyArray(std::vector<std::vector<double>> const& dvecvec)
{
    ...
    PyObject* parr = PyArray_FROM_OTF(plist, NPY_DOUBLE, NPY_ARRAY_IN_ARRAY);
    Py_DECREF(plist);
    return parr;
}
...
static std::vector<std::string> asStrVec(PyObject* pobj)
{
    ...
    PyObject* arr = PyArray_FROM_OTF(pobj, NPY_STRING, NPY_ARRAY_IN_ARRAY | NPY_ARRAY_FORCECAST);
...
static PyObject* asPyArray(std::vector<std::string> const& svec)
{
  ...
  PyObject* parr = PyArray_FROM_OTF(plist, NPY_UNICODE, NPY_ARRAY_IN_ARRAY);
  Py_DECREF(plist);
  return parr;
}
