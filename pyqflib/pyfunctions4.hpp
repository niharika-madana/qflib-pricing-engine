...
PY_BEGIN;
  ...
  if (!PyArg_ParseTuple(pyArgs, "OOOOOOOOOOOO", &pyPayoffType, &pyStrike, &pyTimeToExp,
    &pySpot, &pyDiscountCrv, &pyGrowthCrv, &pyDivYield, &pyVolatility,
    &pyFxVol, &pyCorrel, &pyPdeParams, &pyAllResults))
    return NULL;
  ...
  QF_ASSERT(spdiscyc, "error: yield curve " + discName + " not found");

  std::string growthName = asString(pyGrowthCrv);
  qf::SPtrYieldCurve spgrowthyc = qf::market().yieldCurves().get(growthName);
  QF_ASSERT(spgrowthyc, "error: yield curve " + growthName + " not found");
  ...
  // create the product
  qf::SPtrProduct spprod(new qf::EuropeanCallPut(payoffType, strike, timeToExp));
  // create the PDE solver with the quanto constructor
  qf::Pde1DResults results;
  qf::Pde1DSolver solver(spprod, spgrowthyc, spdiscyc, spot, divYield, spvol,
                         fxVol, correl, results);
  solver.solve(pdeparams);
